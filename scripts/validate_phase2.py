...
for name, func, seed, params in maps:
    ...
    fig.tight_layout()
    path = os.path.join(FIGS, f"phase2_orbit_{name.lower().replace(' ', '_').replace('(', '').replace(')', '')}.png")
...
fig.tight_layout()
path = os.path.join(FIGS, "phase2_henon_histogram.png")
fig.savefig(path, dpi=150)
plt.close(fig)
print(f"  Saved {path}")
